...
@TargetClass(
    value = ClassPathScanner.class,
    onlyWith = ClassPathScannerSubstitutions.IsJBossVFSAbsent.class)
public final class ClassPathScannerSubstitutions {
  ...
  private ClassPathLocationScanner createLocationScanner(String protocol) {
    ...
    if ("jar".equals(protocol)) {
      String separator = "!/";
      ClassPathLocationScanner locationScanner =
          (ClassPathLocationScanner)
              (Object) new JarFileClassPathLocationScannerSubstitutions(separator);
      locationScannerCache.put(protocol, locationScanner);
      resourceNameCache.put(locationScanner, new HashMap<>());
      return locationScanner;
    }
