...
public final class NativePathLocationScanner implements ResourceAndClassScanner {
  ...
  private static Collection<String> applicationMigrationFiles =
      Collections.emptyList(); // the set default to aid unit tests
  private static Collection<Class<? extends JavaMigration>> applicationMigrationClasses =
      Collections.emptyList(); // the set default to aid unit tests
  private static Map<String, Collection<Callback>> applicationCallbackClasses =
      Collections.emptyMap(); // the set default to aid unit tests
  ...
  public NativePathLocationScanner(Collection<Location> locations) {
    ...
    for (String migrationFile : applicationMigrationFiles) {
      if (isClassPathResource(locations, migrationFile)) {
        ...
        scannedResources.add(
            new ClassPathResource(null, migrationFile, classLoader, StandardCharsets.UTF_8));
        ...
        Collection<LoadableResource> resources =
            fileSystemScanner.scanForResources(new Location(migrationFile));
